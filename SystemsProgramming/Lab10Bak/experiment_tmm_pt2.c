...
int main(int argc, char *argv[])
{
    ...
    ok = 1;
    progname = argv[0];
    while ((ch = getopt(argc, argv, "hm:n:p:qs:t:v")) != -1) {
        switch (ch) {
	...
	/*
		QUESTION (small detail)
		Am I supposed to use a max function? I figure this is what you want anyway
	*/

	if(nThreads > 1)
	{
		matMulThreadStats = (MatMulThreadStats*)malloc(sizeof(MatMulThreadStats) * nThreads);
	}
	else
	{
	...
	/*
		QUESTION (small detail)
		mat_mul() already takes these arguments... right?
	*/
	...
	if(nThreads == 0)
	{
		printf("%lf", matMulThreadStats[0].cpuTime);
	}
	else
	{
		cpuTimeSum = 0.0;
		...
		{
			printf("%lf", matMulThreadStats[i].cpuTime);
			cpuTimeSum += matMulThreadStats[i].cpuTime;
			/*QUESTION*/
			printf("%d", i * n); // n is number of rows?
			// Modify (pull number of rows from matmulthreadstats
		}
		printf("%lf", cpuTimeSum);
	}
